...
class LSM6DS3:
    ...
    def __init__(self,
        ...
        if enable_acc:
            ...
            if (ACC_ODR <= self.ACC_ODR_6_66_KHZ and ACC_ODR >= self.ACC_ODR_12_5_HZ):

                self.ACC_ODR = ACC_ODR

            else:

                print('%s: Invalid output data rate specified for accelerometer, \
                      setting to 12.5 HZ so module can still run, to change use changeAccODR method' % self.__name__)

                self.ACC_ODR = self.ACC_ODR_12_5_HZ

        if enable_gyro:

            if (GYRO_ODR <= self.GYRO_ODR_1_66_KHZ and GYRO_ODR >= self.GYRO_ODR_12_5_HZ):

                self.GYRO_ODR = GYRO_ODR

            else:

                print('%s: Invalid output data rate specified for gyroscope, \
                      setting to 12.5 HZ so module can still run, to change use changeGyroODR method' % self.__name__)

                self.GYRO_ODR = self.GYRO_ODR_12_5_HZ

        # flag for enabling accelerometer
        self.acc_enabled = (enable_acc is not None)
        ...
        self.gyro_enabled = (enable_gyro is not None)
        ...
        if acc_scale == self.ACC_SCALE_2G:
            self.acc_scale = 1
        elif acc_scale == self.ACC_SCALE_4G:
            self.acc_scale = 2
        elif acc_scale == self.ACC_SCALE_8G:
            self.acc_scale = 4
        elif acc_scale == self.ACC_SCALE_16G:
            self.acc_scale = 8
        ...
        # this will be a constant multiplier for the output data
        if gyro_scale == self.GYRO_SCALE_250DPS:
            self.gyro_scale = 2
        elif gyro_scale == self.GYRO_SCALE_500DPS:
            self.gyro_scale = 4
        elif gyro_scale == self.GYRO_SCALE_1000DPS:
            self.gyro_scale = 8
        elif gyro_scale == self.GYRO_SCALE_2000DPS:
            self.gyro_scale = 16
    ...
    def enableAccInterrupt(self, pin=ACC_INTERRUPT_PIN_INT1):
        ...
        if pin == self.ACC_INTERRUPT_PIN_INT1:
        ...
        elif pin == self.ACC_INTERRUPT_PIN_INT2:
    ...
    def enableGyroInterrupt(self, pin=GYRO_INTERRUPT_PIN_INT2):
        ...
        if pin == self.GYRO_INTERRUPT_PIN_INT1:
        ...
        elif pin == self.GYRO_INTERRUPT_PIN_INT2:
    ...
    def disableGyroInterrupt(self):
        ...
        # nothing to do, already disabled
        if not self.gyro_interrupt_enabled and self.__initialized:
            return
